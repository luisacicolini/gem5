...
main(void)
{
    ...
    /*
     * Setup: generate the real magic/shift pair for each divisor. Not
     * measured -- do_raw is the hot path in real use (called once per
     * division), gen() runs once per divisor up front.
     */
    for (int d = 0; d < NUM_DIVISORS; d++) {
        libdivide_internal_s64_gen(divisors[d], 0, &magics[d]);
    }
    ...
    for (int d = 0; d < NUM_DIVISORS; d++) {
        for (int i = 0; i < NUM_NUMERATORS; i++) {
            libdivide_s64_do_raw(numerators[i], magics[d].magic,
                                 magics[d].more, &results[d][i]);
        }
    }
    m5_dump_stats(0, 0);
